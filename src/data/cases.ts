...
export const casesData: CaseData[] = [
  {
    ...
    descriptionRu: [
      'Клиент — OXYTERRA, производитель кислородного оборудования для медицинских и восстановительных процедур. До нас уже была попытка с другой командой — результат не устроил. Мы посмотрели предыдущую версию, поняли проблемы и предложили своё видение: два ролика — эмоциональный бренд-фильм про идею кислорода и жизни, и технический ролик с объяснением аппарата и интерфейса.',
      'Ключевой момент в работе: мы согласовывали с клиентом каждую сцену до запуска производства — это сократило правки и сделало процесс предсказуемым.',
      'Иногда полностью пересобирали графические сцены, потому что понимали: можно сделать сильнее. Именно такие проекты нам нравятся больше всего.',
    ],
    descriptionEn: [
  ...
  },
  {
    ...
    descriptionRu: [
      'Это был наш первый собственный AI-проект — без клиента и бюджета, только с идеей: сделать документальный фильм настолько хорошим, чтобы стало понятно — AI уже работает как полноценный инструмент кино.',
      'Концепция, сценарий, персонажи, озвучка, монтаж — всё с нуля. Изначально планировался ролик на минуту, но история выросла до полноценного фильма на 2,5 минуты.',
      'Больше всего времени ушло не на картинки, а на эмоцию: правильная мимика, живой голос, синхрон губ, звук улиц, музыка, детали. Именно после этого проекта появился After Human.',
    ],
    descriptionEn: [
  ...
  },
  {
    ...
    descriptionRu: [
      'Второй собственный проект — проверка: можем ли мы рассказать полноценную историю без закадрового голоса, только через визуал, звук и монтаж?',
      'Мы взяли знакомую историю про Джинна и перевернули её. Вместо волшебства — обычная квартира, пицца, телевизор и мечта сбежать на Бали. Сложнее всего было сохранить персонажа узнаваемым и при этом показать трансформацию: в обычной жизни герой выглядит проще и грубее, а в своих фантазиях — другим человеком.',
      'Получилось тёплое кино, которое хочется досмотреть до конца. Один из тех проектов, где мы по-настоящему кайфовали от каждой сцены.',
    ],
    descriptionEn: [
